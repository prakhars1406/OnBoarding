...
func AddNewMerchantConfig(datastoreClient factory.MongoClient) http.HandlerFunc {
	return func(writer http.ResponseWriter, request *http.Request) {
		...
		var merchantConfig model.MerchantConfigDetails
		err := json.NewDecoder(request.Body).Decode(&merchantConfig)
		if err != nil {
			err := json.NewEncoder(writer).Encode(struct {
				Result  bool   `json:"result"`
				Error string `json:"err,omitempty"`
		...
		if merchantId!=merchantConfig.MerchantId{
			err := json.NewEncoder(writer).Encode(struct {
				Result  bool   `json:"result"`
				Error string `json:"err,omitempty"`
			}{Result: false, Error: "invalid merchant id"})
		...
		_, err = datastoreClient.AddNewMerchantConfig(merchantConfig)
		if err != nil {
			utility.HandleError(writer, http.StatusBadRequest, err)
		}  else {
			err := json.NewEncoder(writer).Encode(struct {
				Message string `json:"result,omitempty"`
			}{Message: "merchant config added successfully"})
			if err != nil {
